...
D3IL_ROOT = os.path.join(PROJECT_ROOT, 'd3il')
GYM_AVOIDING_ROOT = os.path.join(D3IL_ROOT, 'environments', 'd3il', 'envs', 'gym_avoiding_env')
for _path in (PROJECT_ROOT, D3IL_ROOT, GYM_AVOIDING_ROOT):
    ...
    for halfspace_variant in halfspace_variants:
        ...
        for seed in seeds:
            ...
            try:
                ...
                # Get model - wrap in try-catch for CUDA errors
                try:
                ...
                for variant_idx, variant in enumerate(projection_variants):
                    print(f'\n--- Running {exp} - {halfspace_variant} - {variant} (seed {seed}) ---')
                    ...
                    print(f'Success rate: {np.mean(n_success):.2f}')
                    print(f'Constraints satisfied: {np.mean(collision_free_completed):.2f}')
                    print(f'Success rate (goal and constraints): {np.mean(n_success_and_constraints):.2f}')
