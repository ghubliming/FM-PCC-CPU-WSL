...
def load_diffusion(*loadpath, epoch='latest', device='cuda:0', seed=None):
    ...
    # Ensure configs use the requested device instead of whatever was saved in the pickle
    for config in [dataset_config, model_config, diffusion_config, trainer_config]:
        if hasattr(config, '_device'):
